...
BEGIN
  ...
  ELSE
    SELECT COUNT(*) INTO NUM FROM BTL1M1.CHANNEL@DBL_M1 CH
    WHERE CH.CHANNELID=VCHANNELID;
...
UNION 
SELECT CHANNELID, CNAME, GET_AWARD(CHANNELID) FROM BTL1M1.CHANNEL@DBL_M1;
...
BEGIN
  ...
  IF (CH_IS_EXISTS > 0) THEN
    ...
    END LOOP;
  ELSE
    SELECT COUNT(*) INTO CH_IS_EXISTS FROM BTL1M1.CHANNEL@DBL_M1 CH
    WHERE CH.CHANNELID=VCHANNELID;
    ...
    FOR R_VIDEO IN (
                                SELECT V.VIDEOID, COUNT(*) AS LIKES
                                FROM  BTL1M1.LIKEVIDEO@DBL_M1 LV, BTL1M1.VIDEO@DBL_M1 V  
                                WHERE V.CHANNELID=VCHANNELID AND LV.VIDEOID = V.VIDEOID
                                   AND EXTRACT(YEAR FROM LV.CREATED) = EXTRACT(YEAR FROM SYSDATE) - 1
                                GROUP BY V.VIDEOID
                              ) 
      LOOP
          UPDATE BTL1M1.VIDEODETAILS@DBL_M1 SET CASH = CASH + R_VIDEO.LIKES * CASH_AMOUNT WHERE VIDEOID=R_VIDEO.VIDEOID;
      END LOOP;
    END IF;
  END IF;
...
UNION
SELECT CH.CHANNELID, CASH 
FROM BTL1M1.VIDEODETAILS@DBL_M1 VD, BTL1M1.VIDEO@DBL_M1 V, BTL1M1.CHANNEL@DBL_M1 CH
WHERE VD.VIDEOID=V.VIDEOID AND V.CHANNELID=CH.CHANNELID  AND CH.CHANNELID = 'ch003';
...
SELECT USERID, REGDAY FROM USERACCOUNT WHERE USERID = 'u002';
SELECT CHANNELID, OWNERID, CREATED FROM CHANNEL WHERE OWNERID='u002';

UPDATE CHANNEL SET CREATED='12-06-2016' WHERE CHANNELID='ch002';
INSERT INTO CHANNEL VALUES('ch050','Test Channel','u002','Viet Nam','12-6-2016');

UPDATE USERACCOUNT SET REGDAY='12-8-2020' WHERE USERID='u002';

...
FROM 
  (
    ...
    FROM BTL1M2.LIKEVIDEO L, BTL1M2.VIDEO V
    WHERE V.VIDEOID=L.VIDEOID AND EXTRACT(YEAR FROM V.CREATED)=2020
    GROUP BY V.VIDEOID, TITLE
    UNION
    SELECT V.VIDEOID, TITLE, COUNT(*) AS LIKES
    FROM BTL1M1.LIKEVIDEO@DBL_M1 L, BTL1M1.VIDEO@DBL_M1 V
    WHERE V.VIDEOID=L.VIDEOID AND EXTRACT(YEAR FROM V.CREATED)=2020
    GROUP BY V.VIDEOID, TITLE
  ) 
ORDER BY LIKES DESC FETCH FIRST 1 ROWS ONLY;
...
(
    ...
    FROM BTL1M2.CHANNEL CH, BTL1M2.SUBSCRIBE S, BTL1M2.USERACCOUNT U
...
(
    SELECT U.USERID, USERNAME
    FROM BTL1M1.CHANNEL@DBL_M1 CH, BTL1M1.SUBSCRIBE@DBL_M1 S, BTL1M1.USERACCOUNT@DBL_M1 U
    WHERE CH.CHANNELID=S.CHANNELID AND S.USERID=U.USERID AND CH.CNAME='Con Nguoi'
        AND EXTRACT(YEAR FROM S.CREATED) = EXTRACT(YEAR FROM SYSDATE)
    INTERSECT
    SELECT U.USERID, USERNAME
...
FROM (
  ...
  FROM BTL1M2.CHANNEL CH, BTL1M2.USERACCOUNT U, (SELECT V.VIDEOID, CHANNELID, COUNT(*) AS LIKES 
                                                      FROM BTL1M2.VIDEO V, BTL1M2.LIKEVIDEO L
                                                      WHERE V.VIDEOID=L.VIDEOID
                                                      GROUP BY V.VIDEOID, CHANNELID) L
  WHERE CH.OWNERID=U.USERID AND CH.CHANNELID=L.CHANNELID
  UNION
  SELECT CH.CHANNELID, CH.CNAME, USERNAME, CH.CREATED, LIKES
  FROM BTL1M1.CHANNEL@DBL_M1 CH, BTL1M1.USERACCOUNT@DBL_M1 U, (SELECT V.VIDEOID, CHANNELID, COUNT(*) AS LIKES 
                                                      FROM BTL1M1.VIDEO@DBL_M1 V, BTL1M1.LIKEVIDEO@DBL_M1 L
                                                      WHERE V.VIDEOID=L.VIDEOID
                                                      GROUP BY V.VIDEOID, CHANNELID) L
  WHERE CH.OWNERID=U.USERID AND CH.CHANNELID=L.CHANNELID
)
...
ORDER BY SUM(LIKES) DESC FETCH FIRST 1 ROWS ONLY;

--5 In ra kenh co so luong dang ky tang len nhieu nhat trong nam
SELECT * FROM (
  SELECT CH.CHANNELID, CH.CNAME, COUNT(*) AS SUBSCRIBES
  FROM BTL1M2.CHANNEL CH, BTL1M2.SUBSCRIBE S, DUAL
  WHERE CH.CHANNELID=S.CHANNELID AND EXTRACT(YEAR FROM S.CREATED)=EXTRACT(YEAR FROM SYSDATE)
  GROUP BY CH.CHANNELID, CH.CNAME
  UNION
  SELECT CH.CHANNELID, CH.CNAME, COUNT(*) AS SUBSCRIBES
  FROM BTL1M1.CHANNEL@DBL_M1 CH, BTL1M1.SUBSCRIBE@DBL_M1 S
  WHERE CH.CHANNELID=S.CHANNELID AND EXTRACT(YEAR FROM S.CREATED)=EXTRACT(YEAR FROM SYSDATE)
  GROUP BY CH.CHANNELID, CH.CNAME
) ORDER BY SUBSCRIBES DESC FETCH FIRST 1 ROWS ONLY;

--6 In ra do tuoi trung binh cua nguoi dung
SELECT TRUNC(AVG(AGE),2) AS AVG_AGE FROM (
  SELECT TRUNC(MONTHS_BETWEEN(SYSDATE, U.BIRTHDAY) / 12) AGE FROM BTL1M2.USERACCOUNT U, DUAL
  UNION
  SELECT TRUNC(MONTHS_BETWEEN(SYSDATE, U.BIRTHDAY) / 12) AGE  FROM BTL1M1.USERACCOUNT@DBL_M1 U, DUAL
);

--7 In ra 3 kenh co tong dung luong luu tru video lon nhat
SELECT * FROM (
  SELECT CH.CHANNELID, CNAME, SUM(FILESIZE) FILESIZE 
  FROM BTL1M2.CHANNEL CH, BTL1M2.VIDEO V, BTL1M2.VIDEODETAILS VD
  WHERE CH.CHANNELID = V.CHANNELID AND VD.VIDEOID=V.VIDEOID
  GROUP BY CH.CHANNELID, CNAME
  UNION
  SELECT CH.CHANNELID, CNAME, SUM(FILESIZE) FILESIZE 
  FROM BTL1M1.CHANNEL@DBL_M1 CH, BTL1M1.VIDEO@DBL_M1 V, BTL1M1.VIDEODETAILS@DBL_M1 VD
  WHERE CH.CHANNELID = V.CHANNELID AND VD.VIDEOID=V.VIDEOID
  GROUP BY CH.CHANNELID, CNAME
) 
ORDER BY FILESIZE DESC FETCH FIRST 1 ROWS ONLY;
...
FROM 
  (
    ...
    FROM BTL1M2.TAG T, BTL1M2.TAG_VIDEO TV,
      (
        SELECT VIDEOID, COUNT(*) LIKES
        FROM BTL1M2.LIKEVIDEO
        GROUP BY VIDEOID
      ) L
    WHERE T.TAGID=TV.TAGID AND TV.VIDEOID=L.VIDEOID
    UNION
    SELECT TV.TAGID, TAGNAME, LIKES
    FROM BTL1M1.TAG@DBL_M1 T, BTL1M1.TAG_VIDEO@DBL_M1 TV,
      (
        SELECT VIDEOID, COUNT(*) LIKES
        FROM BTL1M1.LIKEVIDEO@DBL_M1
        GROUP BY VIDEOID
      ) L
    WHERE T.TAGID=TV.TAGID AND TV.VIDEOID=L.VIDEOID
  )
GROUP BY TAGID, TAGNAME
ORDER BY LIKES FETCH FIRST 1 ROWS ONLY;
...
FROM 
  (
    ...
    FROM BTL1M2.VIDEO V, BTL1M2.TAG T, BTL1M2.TAG_VIDEO TV, BTL1M2.LIKEVIDEO LV
    WHERE TV.VIDEOID=V.VIDEOID AND TV.TAGID=T.TAGID AND V.VIDEOID = LV.VIDEOID
    GROUP BY T.TAGID, V.VIDEOID, TAGNAME, TITLE
    UNION
    SELECT T.TAGID, TAGNAME, V.VIDEOID, TITLE, COUNT(*) LIKES
    FROM BTL1M1.VIDEO@DBL_M1 V, BTL1M1.TAG@DBL_M1 T, BTL1M1.TAG_VIDEO@DBL_M1 TV, BTL1M1.LIKEVIDEO@DBL_M1 LV
    WHERE TV.VIDEOID=V.VIDEOID AND TV.TAGID=T.TAGID AND V.VIDEOID = LV.VIDEOID
    GROUP BY T.TAGID, V.VIDEOID, TAGNAME, TITLE
  ) T1
  ...
  (
    ...
    FROM 
      (
        ...
        UNION
        SELECT TV.TAGID, LIKES
        FROM BTL1M1.TAG_VIDEO@DBL_M1 TV,
          (
            SELECT VIDEOID, COUNT(*) LIKES
            FROM BTL1M1.LIKEVIDEO@DBL_M1
            GROUP BY VIDEOID
...
--10 In danh sach cac video co 2 tag 'hai','thi phi' thuoc cac kenh tai 'Viet Nam' hoac 'Nhat Ban'
SELECT DISTINCT VIDEOID, TITLE
FROM
  (
    SELECT V.VIDEOID, TITLE, TV.TAGID 
    FROM BTL1M2.VIDEO V, BTL1M2.CHANNEL CH, BTL1M2.TAG_VIDEO TV
    WHERE V.CHANNELID=CH.CHANNELID AND V.VIDEOID=TV.VIDEOID AND CH.NATION IN ('Viet Nam','Nhat Ban')
    UNION
    SELECT V.VIDEOID, TITLE, TV.TAGID 
    FROM BTL1M1.VIDEO@DBL_M1 V, BTL1M1.CHANNEL@DBL_M1 CH, BTL1M1.TAG_VIDEO@DBL_M1 TV
    WHERE V.CHANNELID=CH.CHANNELID AND V.VIDEOID=TV.VIDEOID AND CH.NATION IN ('Viet Nam','Nhat Ban')
  ) V1
WHERE NOT EXISTS (
  SELECT * FROM BTL1M2.TAG T
